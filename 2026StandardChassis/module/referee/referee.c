...
USART_t *referee_usart_instance;				  // 串口实例
static supervisor_t *referee_supervisor_instance; // 监视器实例

static USART_t *refereepic_usart_instance;			 // 串口实例
...
void RefereeSolve(uint8_t *data)
{
    ...
    uint16_t offset_frame_tail = (data[Offset_SOF_DataLength + 1] << 8) + data[Offset_SOF_DataLength] + LEN_CMD_ID + LEN_FRAME_HEAD;
    if (offset_frame_tail > REFEREE_RXBUFF_SIZE)
    {
        return; // 数据长度不合法，直接丢弃，防止越界
    }
    uint16_t cmd_id = (data[Offset_cmd_ID + 1] << 8 | data[Offset_cmd_ID]);

    if (data[0] != 0xA5 || !Verify_CRC16_Check_Sum(data, offset_frame_tail + 2) || !Verify_CRC8_Check_Sum(data, LEN_FRAME_HEAD))
	{
		return;
	}
...
void RefereePicSolve(uint8_t *data)
{
	...
	case ID_get_channel:
		...
		break;
// #ifndef USE_REMOTE_KEYBOARD
// 	case ID_keyboard_information:
// 		memcpy(&(keyboard_data), (data + Offset_data), LEN_keyboard_information);
// 		break;
// #endif
	}
	if (data[offset_frame_tail + 2] == 0xA5)
