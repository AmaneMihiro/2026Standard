...
float chassis_max_power = 55; // 底盘最大功率 , 5~8W 的误差
...
void chassis_power_control(void)
{
    ...
    {
        speed_rpm = chassis_motor_drive[i]->measure.speed; // motor_list[i]->data.speed ---> rpm

        P_cal[i] = K0 * chassis_motor_drive[i]->target.current * speed_rpm +
                   K1 * speed_rpm * speed_rpm +
                   K2 * chassis_motor_drive[i]->target.current * chassis_motor_drive[i]->target.current +
                   constant; // 静态功耗
    ...
    if (P_total > chassis_max_power) // 总功率超过
    {
        ...
        for (uint8_t i = 0; i < motor_index; i++) 
        {
            speed_rpm = chassis_motor_drive[i]->measure.speed; // motor_list[i]->data.speed ---> rpm  , 注意获取新的电机转速
            ...
            else if (chassis_motor_drive[i]->target.current > 0) // 根据电机原本转向分配
            {
